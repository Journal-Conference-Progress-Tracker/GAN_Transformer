...
for generation_size in generation_sizes:
    ...
    for size in sample_sizes:
        ...
        for train_index, val_index in skf.split(X_train, y_train):
            ...
            smote_augmented_x = np.concatenate([X_train_fold, synthetic_x_smote])
            smote_augmented_y = np.concatenate([y_train_fold, synthetic_y_smote])
            smote_loader = xy_to_tensordataset(smote_augmented_x, smote_augmented_y, return_loader=True)
            ...
            kde_augmented_x = np.concatenate([X_train_fold, synthetic_x_kde])
            kde_augmented_y = (np.concatenate([y_train_fold, synthetic_y_kde])
                               if synthetic_y_kde is not None else y_train_fold)
            kde_loader = xy_to_tensordataset(kde_augmented_x, kde_augmented_y, return_loader=True)
            
            # GMM
            synthetic_x_gmm, synthetic_y_gmm = conditional_gmm_sampling(
                X_train_fold, y_train_fold, generation_size, condition=[0, 1, 2]
            )
            gmm_augmented_x = np.concatenate([X_train_fold, synthetic_x_gmm])
            gmm_augmented_y = (np.concatenate([y_train_fold, synthetic_y_gmm])
                               if synthetic_y_gmm is not None else y_train_fold)
            gmm_loader = xy_to_tensordataset(gmm_augmented_x, gmm_augmented_y, return_loader=True)
            ...
            gan_augmented_x = np.concatenate([X_train_fold, synthetic_x_gen])
            gan_augmented_y = np.concatenate([y_train_fold, synthetic_y_gen])
            gan_loader = xy_to_tensordataset(gan_augmented_x, gan_augmented_y, return_loader=True)
            ...
            smote_acc_knn  = KNN().fit_and_eval(smote_augmented_x, smote_augmented_y, X_val_fold, y_val_fold)
            kde_acc_knn    = KNN().fit_and_eval(kde_augmented_x, kde_augmented_y, X_val_fold, y_val_fold)
            gmm_acc_knn    = KNN().fit_and_eval(gmm_augmented_x, gmm_augmented_y, X_val_fold, y_val_fold)
            gan_acc_knn    = KNN().fit_and_eval(gan_augmented_x, gan_augmented_y, X_val_fold, y_val_fold)
            ...
            smote_acc_rf  = RF().fit_and_eval(smote_augmented_x, smote_augmented_y, X_val_fold, y_val_fold)
            kde_acc_rf    = RF().fit_and_eval(kde_augmented_x, kde_augmented_y, X_val_fold, y_val_fold)
            gmm_acc_rf    = RF().fit_and_eval(gmm_augmented_x, gmm_augmented_y, X_val_fold, y_val_fold)
            gan_acc_rf    = RF().fit_and_eval(gan_augmented_x, gan_augmented_y, X_val_fold, y_val_fold)
